...
const fetchRecipes = async () => {
  ...
  // Ingredients ko simple space se join karein
  const query = ingredients.join(" "); 
  
  // App IDs - Bilkul saaf (no spaces, no dashes)
  const APP_ID = "c0565c16";
  const APP_KEY = "7b098fcfe5774321fce04a8f39230d42";
  ...
  try {
    ...
    if (!response.ok) {
      // Agar error 401 hai to key ka masla hai, 404 hai to URL ka
    ...
    if (!data.hits || data.hits.length === 0) {
      alert("No recipes found! Try simple ingredients like 'Chicken' or 'Pasta'.");
      return;
    }
    ...
    const formattedRecipes = data.hits.map((item, index) => ({
      ...
      time: item.recipe.totalTime > 0 ? `${item.recipe.totalTime} min` : "25 min",
      servings: `${item.recipe.yield} Persons`,
      difficulty: item.recipe.ingredients.length > 8 ? "Medium" : "Easy",
      ingredients: item.recipe.ingredientLines,
      youtube: `https://www.youtube.com/results?search_query=${encodeURIComponent(item.recipe.label)}+recipe`,
      instructions: "Check the video guide for full details."
    }));
    ...
    alert("Connection Error! Agar internet sahi hai, to Edamam site par ja kar check karein ke aapki key 'Active' hai ya 'Limit Exceeded' to nahi ho gayi.");
  } finally {
    setLoading(false);
  }
};
